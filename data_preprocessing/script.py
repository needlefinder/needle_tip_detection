...
def labelMapFromNeedle(inputVolume, needleID, value, caseNumber, name):
    ...
    slicer.util.saveNode(outputLabelMap, USERPATH + '/Projects/LabelMaps_%.2f-%.2f-%.2f/%03d/needle-%s.nrrd'%(spacing[0], spacing[1], spacing[2], caseNumber,name))
...
def get_resized_img(k, data_type = sitk.sitkFloat32):
    ...
    dirPath = USERPATH + '/Projects/LabelMaps_%.2f-%.2f-%.2f/%03d/'%(spacing[0], spacing[1], spacing[2], k)
    createDir(dirPath)
    filename = USERPATH + '/Projects/LabelMaps_%.2f-%.2f-%.2f/%03d/case.nrrd'%(spacing[0], spacing[1], spacing[2], k)
...
def extract(k):
    ...
    slicer.util.saveNode(backgroundNode, USERPATH + '/Projects/LabelMaps_%.2f-%.2f-%.2f/%03d/case.nrrd'%(spacing[0], spacing[1], spacing[2], cases[k]))
